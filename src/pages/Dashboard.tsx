...
import {
  Meeting, TranscriptSegment, MeetingSource,
  UserProfile, SubscriptionPlan, PLAN_CONFIGS
} from '../types';
...
const handleLogin = async () => {
  ...
  // Verifica limite de minutos do plano
  const checkMinutesLimit = (): boolean => {
    if (!userProfile) return false;
    if (userProfile.minutesLimit === null) return true; // ilimitado
    return (userProfile.minutesUsed || 0) < userProfile.minutesLimit;
  };

  // Verifica limite de reuniões salvas
  const checkMeetingsLimit = (): boolean => {
    const limit = PLAN_CONFIGS[userProfile?.plan ?? 'starter'].maxSavedMeetings;
    if (limit === null) return true;
    return meetings.length < limit;
  };
  ...
  const PlanBadge = () => {
    ...
    const colors: Record<SubscriptionPlan, string> = {
      starter: 'text-white/20',
      pro_monthly: 'text-blue-400',
      pro_annual: 'text-purple-400',
      power: 'text-amber-400',
    };
    return (
      <span className={cn('text-[9px] font-bold uppercase tracking-widest', colors[plan])}>
      ...
      <div className="px-6 py-2 border-b border-white/5">
        <div className="flex justify-between text-[10px] text-white/20 mb-1">
          ...
          <span className={isWarning ? 'text-amber-400' : ''}>
            {userProfile.minutesUsed || 0} / {userProfile.minutesLimit} min
          </span>
        </div>
        <div className="h-0.5 bg-white/5 rounded-full overflow-hidden">
          <div
            className={cn('h-full rounded-full transition-all', isWarning ? 'bg-amber-400' : 'bg-white/20')}
            style={{ width: `${pct}%` }}
          />
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans flex flex-col relative overflow-hidden">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center space-y-8 relative z-10">
          ...
          <div className="space-y-3">
            ...
            <p className="text-white/40 max-w-xs mx-auto text-sm">
              Transcrições salvas com segurança na nuvem.
            </p>
          </div>
            ...
            <AnimatePresence mode="wait">
              ...
              {view === 'settings' && (
                <motion.div
                  ...
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: -20 }}
                  className="h-[calc(100vh-80px)] overflow-y-auto p-6 max-w-lg mx-auto"
                >
                  ...
                  <div className="glass-card p-5 rounded-2xl border border-white/5 mb-4 space-y-3">
                    ...
                    <button
                      ...
                      className="w-full py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-sm text-white/60 hover:text-white transition-colors flex items-center justify-center gap-2"
                    >
                  ...
                  {userProfile?.plan === 'power' && (
                    <div className="glass-card p-5 rounded-2xl border border-white/5 mb-4 space-y-3">
                      ...
                      <input
                        type="password"
                        defaultValue={userProfile?.apiKey ?? ''}
                        placeholder="gsk_..."
                  ...
                  <button
                    ...
                    className="w-full py-3 rounded-xl border border-red-500/20 text-red-400 hover:bg-red-400/10 transition-colors text-sm flex items-center justify-center gap-2"
                  >
      ...
      <PremiumOverlay
        ...
        currentPlan={userProfile?.plan ?? 'starter'}
      />
    </div>
  );
}
...
function NavButton({ active, icon, label, onClick }: {
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "flex flex-col items-center gap-1 transition-all rounded-xl px-3 py-1",
        active ? "text-white" : "text-white/30 hover:text-white/60"
      )}
    >
      <div className={cn("transition-transform", active && "scale-110")}>{icon}</div>
      <span className={cn("text-[10px] font-medium uppercase tracking-widest transition-opacity", active ? "opacity-100" : "opacity-0")}>
        {label}
      </span>
